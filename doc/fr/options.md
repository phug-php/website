...
L'option *macros* ou la méthode `macro` permettent d'ajouter
des méthodes à la fois à la façade `Phug` et au instances de
`Phug\Renderer` ( et si vous utilisez **Pug-php**, elles
seront aussi ajoutées à la açade `Pug\Facade` et aux instances
de `Pug`).
```php
...
Les classes système peuvent être remplacées
grâce aux options. Cela vous permet d'étendre
les classes et méthodes de **Phug**.
Ces options sont initiales donc vous devez
réinitialiser le *renderer* si vous les
changer après un rendu.

### compiler_class_name `string`

Permet de remplacer le [Compiler](/api/classes/Phug.Compiler.html)

### parser_class_name `string`

Permet de remplacer le [Parser](/api/classes/Phug.Parser.html)

### lexer_class_name `string`

Permet de remplacer le [Lexer](/api/classes/Phug.Lexer.html)
...
Permet de remplacer la [classe Lexer\State](/api/classes/Phug.Lexer.State.html)

### parser_state_class_name `string`

Permet de remplacer la [classe Parser\State](/api/classes/Phug.Parser.State.html)

### node_compilers `array`

Permet de changer les *compilers* pour chaque
type de nœud, la map par défaut est :
...
Permet de changer le formattage pour chaque type
d'élément, la map par défaut est :
```php
[
...
Où `$this` est l'instance de formattage courante.

### token_handlers `array`

Permet de changer la manière dont les *tokens*
du *lexer* sont parsés, la map par défaut est :
```php
[
...
Permet de changer la manière de scanner et détecter
chaque séquence de caractères pour obtenir le *token*
correspondant, la map par défaut est :
```php
[
...
Permet de changer la manière de gérer les assignements
...
Permet de changer la manière de gérer les attributs,
exemple :
